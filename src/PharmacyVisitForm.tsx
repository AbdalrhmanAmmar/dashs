...
import { CalendarIcon, Store, Receipt, Upload, DollarSign } from "lucide-react";
...
export default function PharmacyVisitForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const pharmaciesData = JSON.parse(
      localStorage.getItem("pharmaciesData") || "[]"
    );

    // Generate unique group ID for this pharmacy visit
    const groupId = `${formData.pharmacyName}-${
      formData.visitDate
    }-${Date.now()}`;
    ...
    localStorage.setItem("pharmaciesData", JSON.stringify(pharmaciesData));
            ...
            {formData.order === "نعم" && (
              <div className="space-y-4 border-t pt-4">
                ...
                <div className="flex justify-end items-center gap-4 text-lg font-semibold">
                  ...
                  <span>
                    المجموع الكلي للطلبية: {formData.orderAmount} ريال
                  </span>
                </div>
              </div>
            )}
            ...
            >
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span>{formData.pharmacyName}</span>
                </div>
                <div className="flex justify-between">
